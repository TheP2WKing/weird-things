...
public class BlockCookieGrass extends ModBlockBase implements IGrowable {
	public BlockCookieGrass(String modid, String name, CreativeTabs tab, Material material, SoundType sound,
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if (!worldIn.isRemote) {
			...
			if (worldIn.getLightFromNeighbors(pos.up()) < 4
				...
				worldIn.setBlockState(pos, ModBlocks.COOKIE_DIRT.getDefaultState());
			} else {
				if (worldIn.getLightFromNeighbors(pos.up()) >= 9) {
					for (int i = 0; i < 4; ++i) {
						...
						if (iblockstate1.getBlock() == ModBlocks.COOKIE_DIRT
								&& worldIn.getLightFromNeighbors(blockpos.up()) >= 4
								&& iblockstate.getLightOpacity(worldIn, pos.up()) <= 2) {
							worldIn.setBlockState(blockpos, ModBlocks.COOKIE_GRASS.getDefaultState());
						}
					}
				}
			}
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return ModBlocks.COOKIE_DIRT.getItemDropped(ModBlocks.COOKIE_DIRT.getDefaultState(), rand, fortune);
	}
		...
		for (int i = 0; i < 128; ++i) {
			...
			while (true) {
				...
				blockpos1 = blockpos1.add(rand.nextInt(3) - 1, (rand.nextInt(3) - 1) * rand.nextInt(3) / 2,
						rand.nextInt(3) - 1);
				if (worldIn.getBlockState(blockpos1.down()).getBlock() != ModBlocks.COOKIE_GRASS
